...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: admins, error: adminsError } = await supabaseAdmin.from("profiles").select("id").eq("role", "admin").eq("status", "approved");
    if (adminsError) {
      throw adminsError;
    }
    ...
    const notificationBody = agentName ? `${agentName} has submitted a registration request.` : "A user has submitted a registration request.";
    ...
    const { data: insertedRows, error: insertError } = await supabaseAdmin.from("notifications").insert(rows).select("id");
    if (insertError) {
      throw insertError;
    }
    return new Response(JSON.stringify({
