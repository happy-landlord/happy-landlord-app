...
function extractPhone(payload) {
  return payload?.user?.phone || payload?.phone || payload?.data?.phone || payload?.record?.phone || null;
}
function extractOtp(payload) {
  return payload?.sms?.otp || payload?.otp || payload?.token || payload?.data?.otp || payload?.data?.token || null;
}
function extractFallbackMessage(payload) {
  return payload?.sms?.message || payload?.message || payload?.data?.message || null;
}
async function sendClickSendSms(phone, message) {
...
serve(async (req)=>{
  try {
    ...
    const hookSecret = hookSecretRaw?.startsWith("v1,") ? hookSecretRaw.split(",")[1] : hookSecretRaw;
    if (!hookSecret) {
      console.error("AUTH_HOOK_SECRET missing");
      return jsonResponse({
        error: "Hook secret missing"
      }, 500);
    }
    ...
    if (!otp && !fallbackMessage) {
      ...
      return jsonResponse({
        error: "Missing OTP or message"
      }, 400);
    }
    const message = otp ? `Your Happy Landlord verification code is: ${otp}` : fallbackMessage;
