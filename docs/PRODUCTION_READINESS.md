...
| E2E (Detox / Maestro) | ✅ | Three Maestro smoke flows added: `.maestro/login.yaml`, `.maestro/checkout.yaml`, `.maestro/return.yaml`. Run with `maestro test .maestro/<flow>.yaml`. Set `MAESTRO_TEST_EMAIL` + `MAESTRO_TEST_PASSWORD` env vars. |
| Smoke checklist | ✅ | Covered by Maestro flows above |

Recommend extending Maestro flows to cover: scan QR, add property, agent invite.
...
- ✅ ~~Notification icon for Android~~ — done (`expo-notifications` plugin).
- ✅ ~~Basic CI (typecheck + lint on PR via GitHub Actions)~~ — done (`.github/workflows/ci.yml`).
- ✅ ~~Maestro smoke tests for login → checkout → return~~ — done (`.maestro/`).
- ✅ ~~Dark theme or explicitly set `userInterfaceStyle: "light"`~~ — done, set to `"light"`.
- ✅ ~~Replace 9 `console.warn` calls with a structured logger / Sentry breadcrumb~~ — done (`lib/utils/logger.ts`).
- Add pre-commit / lint-staged hook.
- Extend Maestro flows: scan QR, add property, agent invite.
- Add `QueryCache`/`MutationCache` global error handler (toast + Sentry breadcrumb).
